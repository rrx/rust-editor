use editor_core::{grapheme_width, BufferConfig, EndOfLine, IndentSize, IndentStyle};
...
impl ViewCharCollection {
    ...
    pub fn char_length_range(&self, start: usize, end: usize) -> usize {
        ...
        self.elements.as_slice()[a..b]
            .iter()
            .fold(0, |acc, e| acc + e.char_len)
    }
    ...
    pub fn push(&mut self, v: ViewChar) {
        ...
        self.elements
            .push(add_size(v, self.current_r, self.current_lc));
...
pub fn string_to_elements(s: &String, config: &BufferConfig) -> ViewCharCollection {
    use ViewChar::*;
    s.graphemes(true)
        .fold(ViewCharCollection::default(), |mut v, c| match c {
            "\t" => {
